...
async function insertTable(db, tableName, fullPath, idCliente) {
  const records = await getDbfRecords(fullPath);
  const insertQuery = InsertQuery(tableName, records, idCliente);
  await InsertTable(db, tableName, insertQuery);
}

async function processFolder(folderPath, idCliente) {
  try {
    ...
    console.log(`Found ${dbfFiles.length} DBF files.`);
    ...
    for (const file of dbfFiles) {
      ...
      await createTable(db, fileName, fullPath, {}, uniqueKey);
      await insertTable(db, fileName, fullPath, idCliente);

      // Here you would call your DBF reader logic
      // await readAndMigrate(fullPath);
    }
    db.end();
...
async function exportProdutosEstoque(clienteId, outputPath) {
  const db = await connectToDatabase();
  console.log("DBF connection established.");

  const rows = await ListaProdutosEstoque(db, clienteId);
  console.log(
    `Retrieved ${rows.length} products in stock for client ${clienteId}.`
  );
