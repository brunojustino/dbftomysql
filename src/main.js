...
const dbfFolderPath = path.join("C:", "siv", "test");
...
async function insertTable(db, tableName, query, onProgress = null) {
  try {
  ...
  try {
    ...
    for (const file of dbfFiles) {
      ...
      try {
        ...
        try {
          await insertTable(db, fileName, insertQuery, onProgress);
        } catch (insertError) {
          throw insertError;
        }
      } catch (tableError) {
...
// run();
processFolder(dbfFolderPath, 10);
// batchRead();
